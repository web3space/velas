...
const BN = utils.BN;
const broadcastTx = require("../p2p/broadcast-tx.ls");
...
BlockchainDouble.prototype.queueTransaction = function(tx) {
  const ptx = this.pending_transactions;

  const rawtx = "0x" + tx.serialize().toString("hex");
  broadcastTx(this.data, rawtx, function(err) {
    if (err) {
      return console.log("cannot bradcast", err);
    }
