...
const mempool = require("../mempool/mempool.ls");
...
// const addPeer = require("../p2p/add-peer.ls");
// const newTx = require("../p2p/new-tx.ls");
...
StateManager.prototype._queueTransaction = function(method, tx, from, blockNumber, callback) {
  ...
  if (method === "eth_sendRawTransaction" && to.number(tx.gasLimit) > to.number(this.blockchain.blockGasLimit)) {
...
StateManager.prototype.processNextAction = function(override) {
  ...
  } else if (queued.method === "eth_sendRawTransaction") {
...
StateManager.prototype.processCall = function(from, tx, blockNumber, callback) {
  ...
  if (tx.nonce.length === 0) {
    self.validateExpectedNonce(tx, from, next);
  } else {
    next(null, tx);
  }
};
...
/* here is coming new transaction */
StateManager.prototype.processTransaction = function(from, tx, cb) {
  var self = this;

  self.validateExpectedNonce(tx, from, function(err, tx) {
    if (err) {
      return cb(err);
    }

    // this function add tx in pending txs and broadcast it by using gossip protocol
    ...
    /*
    commented due to consensus change
...
StateManager.prototype.validateExpectedNonce = function(tx, from, callback) {
  ...
  this.blockchain.getQueuedNonce(from, (err, expectedNonce) => {
    ...
    if (!tx.isSigned()) {
      return callback(new Error("Cannot process unsigned transaction"));
    }
    ...
    callback(
      new Error(`the tx doesn't have the correct nonce. account has nonce of: ${expected} tx has nonce of: ${actual}`)
    );
  });
};
