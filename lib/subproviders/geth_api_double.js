...
const becomePeer = require("../../p2p/become-peer.ls");
  ...
  this.state.initialize(function(err) {
    ...
    callbacks.forEach(function(callback) {
      setImmediate(function() {
        ...
        // Becoming a peer
        // const db = self.state.blockchain.data;

        // const me = self.state.options;
        console.log("become peer", self.state.options.port);
        becomePeer(self.state, function(err, data) {
          console.log("peer status", err, data);
        });
      });
    });
  });
...
GethApiDouble.prototype.handleRequest = function(payload, next, end) {
  ...
  const methods = Object.keys(self);

  if (method == null) {
    return end(new Error("Method " + payload.method + " is not supported. Available methods: " + methods));
  }
...
GethApiDouble.prototype.addPeer = function(peer, callback) {
  // const db = this.state.blockchain.data;
  // const me = this.state.options;

  addPeer(this.state, peer, callback);
};

// New Transaction from one of registered peers
GethApiDouble.prototype.newTx = function(data, callback) {
  // const db = this.state.blockchain.data;
  // const me = this.state.options;

  newTx(this.state, data, callback);
  // callback("newTx is not implemented yet");
};
